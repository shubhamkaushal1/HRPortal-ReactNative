...
import { leavesApplyTypeApi,ApplyLeaveApi } from '../api/apis';
...
const displayData = async ()=>{  
  try{  
    ...
    leavesApplyTypeApi(jwtToken,setLeaveTypeData); 
  }  
  catch(error){  
    alert(error)  
  }  
...
// Moment(eventss[i].holiday_from_date).format('y-MM-DD HH:MM:ss');
// console.log(value);
const dataToSubmit = {
  leave_type_id: value,
  from_date: Moment(fromdate).format('DD/MM/y'),
  to_date: Moment(todate).format('DD/MM/y'),
  reasons: reason,
}
 const submitData = async() =>{
  let jwtToken = await AsyncStorage.getItem('jwtToken'); 
  let result = await ApplyLeaveApi(jwtToken,dataToSubmit);
      ...
      return (
        <View style={{backgroundColor:'#F4F5F7', marginTop:15, paddingVertical:12,}}>
          <View style={{paddingHorizontal:12}}>
            ...
            <TextInput numberOfLines={4} multiline={true} onChangeText={(val) => {setReason(val)}}/>
          </View>
          <View style={{paddingHorizontal:12}}>
          {/* <TouchableOpacity
